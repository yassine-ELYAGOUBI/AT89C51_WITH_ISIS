...
void main(void){
	...
	LCD_commande(0x06);
	LCD_commande(0x01);	
	LCD_display_str("Enter Password:  ");
	...
	while(1){
		...
		if(key_value != 0){			
			Entred_pass[keyCnt] = key_value;
			LCD_commande(0xC0 + keyCnt);
			LCD_display('*');
			keyCnt = keyCnt + 1;				
		}
		if(keyCnt == 4){			
			LCD_commande(0x01);
			keyCnt = 0;
			passCnt = strcmp(Default_pass,Entred_pass);
			
			if(passCnt == 0){
				device = ~device;
				LCD_commande(0x80);
				LCD_display_str("Correct Password");
				LCD_commande(0xC0);
				LCD_display_str("#:start/*:cancel");
				delay(100);
			}
			else{				
				LCD_commande(0x80);
